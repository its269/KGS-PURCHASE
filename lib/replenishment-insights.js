...
export function buildReplenishmentInsight({
    ...
    if (!hasSalesHistory) {
        ...
        isMain &&
        Number(mainWarehouseContext?.vendorShortfall) === 0 &&
...
function buildHowItWorks({
    ...
    if (hasSalesHistory) {
        ...
        steps.push({
            ...
            text: isMainWarehouseView
                ? `Branch replenishment total: ${formatUnits(mainWarehouseContext.totalBranchReplenishment)} units. Minus MAIN stock (${formatUnits(currentStock)}) = ${formatUnits(suggestedQty)} units to order from vendor. Coming PO (${formatUnits(mainWarehouseContext.comingPO)}) is shown separately.`
