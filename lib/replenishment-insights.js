...
export function buildReplenishmentInsight({
    ...
    if (!hasSalesHistory) {
        ...
        whatToDo = `Plan transfers totaling ${formatUnits(mainWarehouseContext.totalBranchReplenishment)} from MAIN to branches — no vendor order needed.`;
        stockoutRisk = "Low";
...
function buildHowItWorks({
    ...
    if (hasSalesHistory) {
        ...
        steps.push({
            ...
            text: isMainWarehouseView
                ? `Total branch repl.: ${formatUnits(mainWarehouseContext.totalBranchReplenishment)} units. MAIN ${TARGET_DAYS_OF_COVER}-day target: ${formatUnits(mainWarehouseContext.mainTargetStock ?? targetStock)} units. After branch transfers from on-hand MAIN stock, order ${formatUnits(suggestedQty)} from vendor so MAIN still reaches its shelf target. Coming PO (${formatUnits(mainWarehouseContext.comingPO)}) is shown separately and is not subtracted.`
