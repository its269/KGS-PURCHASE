...
export function buildReplenishmentInsight({
    ...
    if (!hasSalesHistory) {
        ...
        headline = "Surplus at MAIN";
        summary = `Branches need ${formatUnits(mainWarehouseContext?.totalBranchReplenishment ?? 0)} units, but MAIN has ${formatUnits(currentStock)} on hand plus ${formatUnits(mainWarehouseContext?.comingPO ?? 0)} incoming — ${formatUnits(Math.abs(suggestedQty))} units above branch need.`;
        whatToDo = `Surplus of ${formatUnits(Math.abs(suggestedQty))} units — no vendor order or transfers needed from MAIN.`;
        stockoutRisk = "Low";
    } else if (
        isMain &&
