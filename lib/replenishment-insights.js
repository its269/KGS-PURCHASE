...
function buildHowItWorks({
    ...
    if (hasSalesHistory) {
        ...
        steps.push({
            ...
            text: isMainWarehouseView
                ? Number(mainWarehouseContext.vendorOrderQty) > 0
                    ? `Vendor PO: ${formatUnits(mainWarehouseContext.vendorOrderQty)} units (MAIN ${TARGET_DAYS_OF_COVER}-day target ${formatUnits(mainWarehouseContext.mainTargetStock ?? targetStock)} after branch transfers). Branch replenishment total: ${formatUnits(mainWarehouseContext.totalBranchReplenishment)}. Coming PO (${formatUnits(mainWarehouseContext.comingPO)}) is shown separately.`
                    : `No vendor PO needed — MAIN has ${formatUnits(currentStock)} on hand. Order qty ${formatUnits(suggestedQty)} = total branch transfer need (${formatUnits(mainWarehouseContext.totalBranchReplenishment)} units across retail branches, e.g. BACOLOD may show its own smaller transfer qty). Coming PO (${formatUnits(mainWarehouseContext.comingPO)}) is shown separately.`
                : `Target stock for ${TARGET_DAYS_OF_COVER} days: ${ads.toFixed(2)} × ${TARGET_DAYS_OF_COVER} = ${formatUnits(targetStock)} units. Suggested order: ${formatUnits(targetStock)} − ${formatUnits(currentStock)} on hand = ${formatUnits(suggestedQty)} units.`,
        });
