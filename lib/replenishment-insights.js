...
export function buildReplenishmentInsight({
    ...
    const metrics = hasSalesHistory
        ? [
              ...
              { label: "Daily sales", value: `${formatUnits(ads)} / day`, hint: `${SALES_LOOKBACK_DAYS}-day average` },
...
function buildHowItWorks({
    ...
    const salesScopeLabel =
        salesScope === "network"
            ? "all retail branches combined (last 90 days)"
            : `branch ${branchId} (last ${SALES_LOOKBACK_DAYS} days)`;
    ...
    steps.push({
        ...
        text: isMainWarehouseView
            ...
            : `This is an active item with low stock (${formatUnits(currentStock)} units) and no sales in the last ${SALES_LOOKBACK_DAYS} days. It is shown as an optional minimum shelf buffer.`,
    });
    ...
    if (hasSalesHistory) {
        steps.push({
            title: "Sells per day",
            text: `${formatUnits(qtySold90)} net units sold in ${SALES_LOOKBACK_DAYS} days ÷ ${SALES_LOOKBACK_DAYS} = ${ads.toFixed(2)} units per day (average daily sales).`,
        });
        steps.push({
            title: "Days left",
    ...
    const preview = isMainWarehouseView
        ...
        : hasSalesHistory
        ? salesScope === "network"
            ? `Based on ${formatUnits(qtySold90)} net sold network-wide in ${SALES_LOOKBACK_DAYS} days; stock checked at MAIN.`
            : `Based on ${formatUnits(qtySold90)} net sold in ${SALES_LOOKBACK_DAYS} days at ${branchId}.`
