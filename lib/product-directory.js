 ...
 * Ported from inventory_welcome.php for Next.js (standalone use by other apps).
 *
 * Env:
...
async function getProduct(productId) {
    ...
    if (custom) {
        return {
            ...
            folder_path: parsePath(custom.folder_path_json),
        };
    }
...
async function createProduct(body = {}) {
    ...
    let folderId = String(body.folder_id || ROOT_ID).trim() || ROOT_ID;
    const id =
        body.id && String(body.id).trim()
            ? String(body.id)
            : `custom_prod_${crypto.randomBytes(6).toString("hex")}`;
    let sku = String(body.sku || "").trim() || id;
