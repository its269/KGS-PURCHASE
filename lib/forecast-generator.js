...
export function monthInvoiceCoverageComplete(ym, dmin, dmax, invoiceCount = 1, asOf = new Date()) {
    ...
    const toYmd = (value) => {
        if (value instanceof Date && !Number.isNaN(value.getTime())) {
            // MySQL DATE at Manila midnight often arrives as previous-day 16:00Z — shift to +08.
            const ph = new Date(value.getTime() + 8 * 60 * 60 * 1000);
    ...
    const minStr = toYmd(dmin);
    const maxStr = toYmd(dmax);
 ...
 * Amount in Buffer = Buffer Inventory × SRP
...
export function computeForecastFields({
    ...
    const bufferAmount = buffer * price;
