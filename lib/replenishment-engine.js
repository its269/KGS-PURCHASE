...
export function buildRecommendation(item, branch, vendorMap, leadTimeMap, recId, lookbackDays = SALES_LOOKBACK_DAYS) {
    ...
    if (hasSalesHistory) {
        daysRemaining = Math.floor(currentStock / ads);
...
export async function computeReplenishmentForBranch(branch, companyId = "main") {
    ...
    if (isMainWarehouse) {
        ...
        // Reuse already-fetched sales map (avoid second sales query)
        const items = await MySqlService.getReplenishmentItems({
            branch,
            companyId: effectiveCompanyId,
            salesMap,
        ...
        for (const item of items) {
            item.salesScope = salesScope;
            const rec = buildRecommendation(item, branch, vendorMap, leadTimeMap, recId++, lookbackDays);
            rec.salesScope = salesScope;
            recommendations.push(rec);
        }
    }
