...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 23;

/**
 * Branch Order qty: Sells/day − (Branch stock + Coming PO).
 * Example (BACOLOD): 7.5 − (19 + 515) = −526.5 (surplus).
...
export function resolveMainOrderQty(
    ...
    return branchRepl - totalAvailable;
}
...
async function computeBranchRecommendations(branchId, companyId, vendorMap, leadTimeMap, startRecId = 2000) {
    ...
    for (const item of items) {
        // Keep per-SKU salesScope from the sales map (do not overwrite with map-level scope)
