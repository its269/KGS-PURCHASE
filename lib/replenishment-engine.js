...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 6;
...
export async function applyLiveComingPo(recommendations, branch) {
    ...
    // Order qty uses live MAIN stock/PO minus that demand total.
    const [comingPoMap, onHandMap, demand] = await Promise.all([
        MySqlService.getOpenPoQtyByItem({ warehouseId: dest }),
        MySqlService.getBranchOnHandMap({ branch: dest, companyId }),
        MySqlService.getAccurateRetailBranchDemandRollup(companyId),
    ]);

    return (recommendations || []).map((rec) => {
        const key = (rec.itemId || "").toUpperCase().replace(/\s+/g, "").trim();
        const comingPoQty = comingPoMap.get(key) || 0;
        const liveOnHand = onHandMap.has(key)
            ? Number(onHandMap.get(key)) || 0
            : null;
        ...
        // Accurate rollup is source of truth — do not fall back to stale cache totals
        // when an item has no retail need (Map miss would otherwise keep inflated values).
        const totalBranchReplenishment = Number(demand.qtyByItem.get(key)) || 0;
...
async function aggregateBranchOrderQty(companyId) {
    ...
    // Bulk accurate demand (live sales + stock + Coming PO). Do not sum stale cache ads.
    const { qtyByItem, salesByItem, branchesNeedingByItem } =
        await MySqlService.getAccurateRetailBranchDemandRollup(companyId);
