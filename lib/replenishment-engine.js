...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 7;
...
export async function applyLiveComingPo(recommendations, branch) {
    ...
    if (!isMain) {
        ...
        return (recommendations || []).map((rec) => {
            ...
            const qtySold90 = Number(rec.qtySold90) || (ads > 0 ? ads * (Number(rec.lookbackDays) || SALES_LOOKBACK_DAYS) : 0);
            ...
            const available = currentStock + comingPoQty;
            const suggestedQty = ads > 0 ? Math.max(0, targetStock - available) : 0;
