...
export function buildRecommendation(item, branch, vendorMap, leadTimeMap, recId, lookbackDays = SALES_LOOKBACK_DAYS) {
...
async function computeBranchRecommendations(branchId, companyId, vendorMap, leadTimeMap, startRecId = 2000) {
    ...
    const { map: salesMap, salesScope, lookbackDays } = await fetchBranchSalesMap(branchId, effectiveCompanyId);
    ...
    for (const item of items) {
        ...
        const rec = buildRecommendation(item, branchId, vendorMap, leadTimeMap, recId++, lookbackDays);
        if (rec) {
...
async function aggregateBranchOrderQty(companyId, vendorMap, leadTimeMap) {
    ...
    return { qtyByItem, branchResults, retailBranches };
}
...
export async function computeReplenishmentForBranch(branch, companyId = "main") {
    ...
    let recommendations = [];
    let salesScope = isMainWarehouse ? "network" : "branch";
    let lookbackDays = SALES_LOOKBACK_DAYS;
    let salesMode = "gross";

    if (isMainWarehouse) {
        const { map: salesMap, salesScope: mainScope, lookbackDays: mainLookback, salesMode: mainMode } =
            await fetchBranchSalesMap("", companyId);
        salesScope = mainScope;
        lookbackDays = mainLookback;
        salesMode = mainMode;
        ...
        const { salesScope: branchScope, lookbackDays: branchLookback, salesMode: branchMode } =
            await fetchBranchSalesMap(branch, effectiveCompanyId);
        salesScope = branchScope;
        lookbackDays = branchLookback;
        salesMode = branchMode;
        recommendations = await computeBranchRecommendations(branch, companyId, vendorMap, leadTimeMap);
    }
