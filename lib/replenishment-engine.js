...
import { SALES_LOOKBACK_DAYS, averageDailySales, toSignedQty } from "@/lib/sales-velocity";
...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 27;

/**
 * Branch Order qty: round(Sells/day − (Branch stock + Coming PO)).
 * Example (BACOLOD): round(7.5 − (19 + 515)) = −527 (surplus).
 * Negative = stock + incoming already covers / exceeds daily sales.
 */
export function resolveBranchOrderQty(sellsPerDay, branchStock, comingPoQty = 0) {
...
/**
 * MAIN Order qty column: branch replenishment need minus MAIN on-hand and incoming PO.
 * Negative = surplus at MAIN (e.g. Papijet Cyan: 756 − (3,147 + 3,000) = −5,391).
 */
export function resolveMainOrderQty(
    ...
    const totalAvailable = mainInv + comingPo;
    return Math.round(branchRepl - totalAvailable);
}
...
export function computeMainRowMetrics({
    ...
    const suggestedQty = resolveMainOrderQty(mainInv, branchRepl, mainTargetStock, comingPoQty);
...
export function buildRecommendation(item, branch, vendorMap, leadTimeMap, recId, lookbackDays = SALES_LOOKBACK_DAYS, comingPO = 0) {
    ...
    if (hasSalesHistory) {
        ...
        suggestedQty = resolveBranchOrderQty(ads, currentStock, comingPoQty);
        const isCritical = suggestedQty > 0 && daysRemaining <= (leadTime + SAFETY_BUFFER_DAYS);
        priority = isCritical ? "High" : suggestedQty > 0 && daysRemaining < 30 ? "Medium" : "Low";
    }
...
function slimBranchAiPatch(rec, { currentStock, comingPoQty, suggestedQty, ads, daysRemaining, priority, lookbackDays }) {
    ...
    if (suggestedQty > 0 && priority === "High") {
        ...
        whatToDo = "No transfer needed — stock + Coming PO exceed Sells/day (surplus).";
    } else if (!whatToDo) {
...
async function applyLiveComingPoFast(recommendations, branch, { slim = false } = {}) {
    ...
    if (!isMain) {
        ...
        return recs.map((rec) => {
            ...
            const available = currentStock + comingPoQty;
            const suggestedQty = ads > 0 ? resolveBranchOrderQty(ads, currentStock, comingPoQty) : 0;
            const daysRemaining = ads > 0 ? Math.floor(available / ads) : null;
            const hasSalesHistory = ads > 0;
            let priority = "Low";
            if (hasSalesHistory) {
                const isCritical = suggestedQty > 0 && daysRemaining !== null && daysRemaining <= (leadTime + SAFETY_BUFFER_DAYS);
                priority = isCritical ? "High" : suggestedQty > 0 && daysRemaining < 30 ? "Medium" : "Low";
            }
...
async function fetchBranchSalesMap(branch, companyId) {
    // Warehouse sites (MNL-MRILAO, etc.) are handled inside getAccurateReplenishmentSalesMap:
    // parent POS invoices only — never catalog-network company-wide totals.
...
async function computeBranchRecommendations(branchId, companyId, vendorMap, leadTimeMap, startRecId = 2000) {
    ...
    for (const item of items) {
        ...
        const qtySold = Number(item.totalQtySold) || 0;
        const comingPo = Number(comingPoMap.get(key)) || 0;
        // Skip inert SKUs: no stock, no incoming PO, no branch sales — noise, not a replenishment need.
        if (onHand <= 0 && comingPo <= 0 && qtySold <= 0) continue;
...
export async function computeReplenishmentForBranch(branch, companyId = "main") {
    ...
    if (isMainWarehouse) {
        ...
        for (const item of items) {
            ...
            const qtySold = Number(item.totalQtySold) || 0;
            const comingPo = Number(comingPoMap.get(key)) || 0;
            // Skip inert SKUs: no stock, no incoming PO, no branch sales.
            if (onHand <= 0 && comingPo <= 0 && qtySold <= 0) continue;
