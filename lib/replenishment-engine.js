...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 12;
...
export function buildMainRecommendation({
    ...
    const availableAtMain = mainInventory; // Coming PO shown separately; do not hide vendor need
    const vendorShortfall = Math.max(0, totalBranchReplenishment - availableAtMain);
    // When MAIN covers demand, still show branch replenishment need as Order qty.
    const orderQty = vendorShortfall > 0 ? vendorShortfall : totalBranchReplenishment;
    const daysRemaining = ads > 0 ? Math.floor((mainInventory + comingPoQty) / ads) : null;
    const isCritical = vendorShortfall > 0 && (
        mainInventory < totalBranchReplenishment ||
        (ads > 0 && daysRemaining !== null && daysRemaining <= (leadTime + SAFETY_BUFFER_DAYS))
    );
    const priority = isCritical ? "High" : vendorShortfall > 0 ? "Medium" : orderQty > 0 ? "Low" : "Low";
...
async function resolveMainBranchDemand(companyId) {
    const [live, cached] = await Promise.all([
        MySqlService.getAccurateRetailBranchDemandRollup(companyId).catch((err) => {
            console.error("[resolveMainBranchDemand] live rollup failed:", err?.message || err);
            return { qtyByItem: new Map(), salesByItem: new Map(), branchesNeedingByItem: new Map() };
        }),
        MySqlService.getRetailBranchDemandRollupFromCache(companyId).catch((err) => {
            console.error("[resolveMainBranchDemand] cache rollup failed:", err?.message || err);
            return { qtyByItem: new Map(), salesByItem: new Map(), branchesNeedingByItem: new Map() };
        }),
    ]);
    ...
    const absorbQty = (src) => {
        if (!src?.qtyByItem) return;
        for (const [rawKey, rawVal] of src.qtyByItem) {
    ...
    const absorbSales = (src) => {
        if (!src?.salesByItem) return;
        for (const [rawKey, rawVal] of src.salesByItem) {
            const key = normalizeInvKey(rawKey);
    ...
    const absorbNeeding = (src) => {
        if (!src?.branchesNeedingByItem) return;
        for (const [rawKey, rawVal] of src.branchesNeedingByItem) {
            const key = normalizeInvKey(rawKey);
            if (!key) continue;
            const n = Number(rawVal) || 0;
            if (n > (branchesNeedingByItem.get(key) || 0)) branchesNeedingByItem.set(key, n);
        }
    };
    ...
    absorbSales(cached);
    absorbNeeding(live);
    absorbNeeding(cached);

    return {
        qtyByItem,
        salesByItem,
        branchesNeedingByItem,
        lookbackDays: live?.lookbackDays || cached?.lookbackDays || SALES_LOOKBACK_DAYS,
        source: qtyByItem.size
            ? `merged:live=${live?.qtyByItem?.size || 0},cache=${cached?.qtyByItem?.size || 0}`
            : "empty",
    };
}
...
export async function applyLiveComingPo(recommendations, branch) {
    ...
    return (recommendations || []).map((rec) => {
        ...
        // Never let a missed rollup key wipe a positive cached total from the row.
        ...
        // Vendor shortfall vs MAIN on-hand. If MAIN already covers branch demand,
        // still surface Total Branch Repl as Order qty so planners see the transfer
        // need instead of a misleading 0 next to non-zero branch demand.
        const vendorShortfall = Math.max(0, totalBranchReplenishment - mainInventory);
        const suggestedQty = vendorShortfall > 0 ? vendorShortfall : totalBranchReplenishment;
        const adsFromDemand = Number(demand.salesByItem.get(key)?.ads) || 0;
        const ads = adsFromDemand || Number(rec.aiInsights?.salesVelocity) || 0;
        const lookbackDays = Number(rec.lookbackDays) || SALES_LOOKBACK_DAYS;
        const qtySold90 =
        ...
        const priority = isCritical ? "High" : vendorShortfall > 0 ? "Medium" : suggestedQty > 0 ? "Low" : "Low";
        const aiInsights = buildReplenishmentInsight({
            itemId: rec.itemId,
            description: rec.description,
            currentStock: mainInventory,
