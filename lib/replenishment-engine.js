...
export const REPLENISHMENT_SALES_LOGIC_VERSION = 18;
...
/**
 * MAIN Order qty column: vendor PO when MAIN must buy, else branch transfer total
 * so planners see the same need as branch rows (e.g. BACOLOD 13 → MAIN 467, not 0).
 */
export function resolveMainOrderQty(mainInventory, totalBranchReplenishment, mainTargetStock) {
    const branchRepl = Number(totalBranchReplenishment) || 0;
    const vendorOrderQty = computeMainVendorOrderQty(
        mainInventory,
...
export function computeMainRowMetrics({
    ...
    const suggestedQty = resolveMainOrderQty(mainInv, branchRepl, mainTargetStock);
    ...
    const priority = isCritical ? "High" : suggestedQty > 0 ? (vendorOrderQty > 0 ? "Medium" : "Low") : "Low";
...
function slimMainAiPatch(rec, metrics, ctx) {
    ...
    const preview = totalBranchReplenishment > 0
        ? `Branches need ${totalBranchReplenishment} units; MAIN has ${mainInventory} + ${comingPoQty} incoming.`
        : (rec.aiInsights?.howItWorks?.preview || `${mainInventory} on hand`);
    let whatToDo;
    if (metrics.vendorOrderQty > 0) {
        whatToDo = `Order ${metrics.suggestedQty} from vendor within 1–2 weeks.`;
    } else if (metrics.suggestedQty > 0) {
        whatToDo = `Plan transfers totaling ${metrics.suggestedQty} from MAIN to branches — no vendor order needed.`;
    } else {
        whatToDo = "No vendor order needed — stock levels are sufficient.";
    }
    return {
        ...(rec.aiInsights || {}),
...
export async function applyLiveComingPo(recommendations, branch, { slim = false } = {}) {
