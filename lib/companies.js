...
/** Remove Damage / Discounted / Warehouse sites from Branch dropdown lists. */
export function filterBranchList(branches) {
    return (branches || []).filter((b) => {
        ...
        return (
            !isExcludedBranchAlias(id) &&
            !isExcludedBranchAlias(nameText) &&
            !isWarehouseLikeAlias(id) &&
            !isWarehouseLikeAlias(nameText)
        );
    });
}
...
export function sqlExcludeBranches(alias = "i") {
    const col = `UPPER(TRIM(${alias}.branch_id))`;
    const likes = EXCLUDED_BRANCH_KEYWORDS.map(() => `${col} NOT LIKE ?`).join(" AND ");
    return {
        clause: `(${alias}.branch_id IS NULL OR TRIM(${alias}.branch_id) = '' OR (${likes}))`,
