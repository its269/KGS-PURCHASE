...
/**
 * Gross outbound sales by inventory ID (invoices + debit memos only).
 * Used for replenishment velocity when returns would hide real demand.
 */
export function aggregateBranchGrossSales(invoices, { branch, startDate, endDate }) {
    const totals = new Map();
    ...
    for (const inv of invoices) {
        ...
        const orderType = getF(inv, "Type") || "Invoice";
        if (orderType === "Credit Memo") continue;
        ...
        for (const line of details) {
            ...
            const amount = Math.abs(Number(getAny(line, "Amount", "ExtendedPrice") || 0));
 ...
 *
 * SalesInvoice payloads can include Credit/Debit Memo types; those are stored
 * separately as CM-* / DM-* from the Invoice entity — skip them for SI-* rows
 * so credit memos are not double-counted in net sales.
 */
export function invoicesToPeriodicSalesRows(invoices, { idPrefix = "SI", defaultOrderType = "Invoice" } = {}) {
    ...
    for (const inv of invoices) {
        ...
        if (prefix === "SI" && (orderType === "Credit Memo" || orderType === "Debit Memo")) {
            continue;
        }
